export default async function postfixToPrefix   () {
    ...
    for (var indexStack = 0; indexStack < inputStack.length; indexStack++) {
      ...
      if (input.innerHTML.match(/[+|\-|*|^|\/]/i)) {
        ...
        $(`.outerAppend${numDiv - 1}`).children().css("background-color","#fdaaaa");
        ...
        $(`.outerAppend${numDiv - 1}`).prepend(`<div class='outExpression numAppended${numOfAppend} scale-up-left'>` + input.innerHTML + `</div>`);
        $(`.outerAppend${numDiv}`).children().appendTo(`.outerAppend${numDiv - 1}`);
        $(`.outerAppend${numDiv}`).remove()
  
        await timer(shortDuration)
        $(`.outerAppend${numDiv - 1}`).children().css("background-color","var(--main-color)");
